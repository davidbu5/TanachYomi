...
export function createMesoraSmallExcelPortraitFromWeeks(weeks: Day[][], yearNum?: number, objectToWriteOn?: any) {
    ...
    wb.write(`${yearNum ? yearNum + " " : ""}RegularSmallPortrait.xlsx`, objectToWriteOn)
}
...
function addSiteHeaderToSheet(wb, sheet, yearNum: number) {
    ...
    sheet.cell(1, 1, 1, 13, true)
        .string(header)
        .style(centerWrappedTextAlign);

    sheet.cell(2, 1, 2, 13, true)
        .string(secondHeader)
        .style(centerWrappedTextAlign);
...
function addHeadToSheet(wb, sheet) {
    ...
    days.forEach((day, index) => {
        ...
        sheet.cell(3, 13 - index).string(day).style(getBorderStyle(wb, true, true, true, true)).style(largerFont)
    })
}
...
function addDayToSheet(wb, sheet, weekIndex: number, dayIndex: number, day: Day) {
    ...
    const weeksInColumn = 28;
    const isFirstColumn = Math.floor(weekIndex / weeksInColumn) === 0;
    const currIndex = 6 - dayIndex;
    const startIndexFromTop = (weekIndex % weeksInColumn) * 3 + 2 + 2;
    const startIndexFromLeft = currIndex + (isFirstColumn ? 7 : 0);
